% To study ACC, test all the electrodes in that ROI separately
...
savedir = fullfile(locations.results_folder,'ACC_theta_dependence_circ_corrcl'); mkdir(savedir);
savedir_R = fullfile(locations.results_folder,'pub_figs','fig4'); mkdir(savedir_R);
coords = load('../data/elecs.mat');

%% Set other parameters

% indices of ACC in the Brainnetome atlas
ROIs = [1 2 11 12 13 14 177 178 179 180 183 184]; % A8m, A9m, A10m, A32p, A24rv, A24cd
...
for pt = locations.subjects
    ...
    feat_elec_indices = feat_elec_indices(fooof_peaks(feat_elec_indices) < 14); % don't analyze any electrodes if they are missing slow oscillations
...
%{                    
                    ...
                    close(f);
%}
                    fprintf('%s %s %s %0.3g: %d significant\n',pt,wave,roi_feat_label,fooof_peaks(feat_elec),sum(clcorr_trials_p_adj<0.05))
...
save(fullfile(savedir_R,'AllPatientsACCPhaseCLcorr.mat'),'clcorr_results')
