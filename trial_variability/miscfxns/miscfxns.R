...
name <- function(x,x.names=x){
	# INPUTS:
	# x: vector or dataframe
	# x.names: names for elements or columns of x - default is to self name 
	# OUTPUTS:
...
collapse.columns <- function(df,cnames=colnames(df),groupby=NULL){
  ...
  df.new <- data.frame(values = as.vector(as.matrix(df[,cnames])),names=as.vector(df.names),stringsAsFactors=F,check.names=F)
  if(!is.null(groupby)){
...
rename.columns <- function(df,cnames,newnames){
  
  cnames.all <- colnames(df)
  df[,newnames] <- df[,cnames]
  return(df[,-which(cnames.all %in% cnames)])
}

get.coef.p.val <- function(m,coef.name){
  return(summary(m)$coef[coef.name,'Pr(>|t|)'])
}

get.coef.beta <- function(m,coef.name){
  return(coef(m)[coef.name])
}
...
maps.to.matrix <- function(maps,dim='row'){
  
  # INPUTS:
  # maps: data frame of roi by number spatial maps
  # dim: 'row' or 'col', whether maps should be oriented with spatial variation along rows or columns
  #
  # OUTPUTS:
  # matrix of maps with maps duplicated along rows or columns
  N <- dim(maps)[1]
  cell.maps.matrix <- lapply(maps,function(map) kronecker(matrix(1,N,1),t(matrix(map,nrow = N,ncol = 1))))
  if(dim=='col'){cell.maps.matrix <- lapply(cell.maps.matrix,t)}
  return(cell.maps.matrix)
}

#############################
### MATLAB to R Utilities ###
#############################

unpack.mat <- function(matvar,fieldname){
  
  # INPUTS:
  # matvar: list from readMat() output
  # fieldname: first level field name in struct
  #
  # OUTPUT: mat struct unpacked down one layer to fieldname
  
  return(matvar[[fieldname]][,,1])
  
}

unpack.mat.struct <- function(matvar,fieldname){
  
  # INPUTS:
  # matvar: list from readMat() output
  # fieldname: field name in struct corresponding to ordered table like struct
  #
  # OUTPUT: mat struct unpackaged with table entries as list names
  
  return(lapply(matvar, function(x) x[,,1][[fieldname]]))
  
}

list.mat.to.vec <- function(l){
  
  # INPUTS:
  # l: list of numeric matrices
  # OUTPUT: vector with the matrix vectorized using as.vector() and then all list elements concatenated
  
  return(Reduce('c',lapply(l,as.vector)))
  
}

unpack.mat.struct.variable <- function(matvar,pts,pts.elecs,fieldname){
  
  return(
    lapply(pts, function(pt) 
    lapply(name(pts.elecs[[pt]]), function(pt.elec) get(fieldname,unpack.mat(unpack.mat(matvar,pt),pt.elec)) ))
    )
  
}

cell.to.vec <- function(l){
  return(unname(unlist(l)))
}
...
make.null.map <- function(maps,non.missing.map=complete.cases(maps)){
  # INPUTS:
  # maps: Nxk spatial brain maps
  # non.missing maps
  maps[non.missing.map,] <- maps[sample(non.missing.map),]
  map.vec.null <- as.data.frame(lapply(maps.to.matrix(maps),as.vector))
  return(map.vec.null)
}
