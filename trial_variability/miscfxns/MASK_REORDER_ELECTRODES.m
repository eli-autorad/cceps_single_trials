...
if isfield(out,'network') % this applies to CCEPs output struct
    ...
    out.network(1).A = out.network(1).A(mask_or_index,mask_or_index);
    out.network(2).A = out.network(2).A(mask_or_index,mask_or_index);
    out.other.periods = out.other.periods(mask_or_index);
