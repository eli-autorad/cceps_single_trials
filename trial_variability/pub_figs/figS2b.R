...
for(wave in waveforms){
  for(pt in pts){
    ...
    if(nrow(df.plt.pt)>0 & !all(is.na(df.plt.pt$spikes.rec))){
      ...
      m.rho.spikes <- lm(rho~log10(D)+gm.stim+gm.rec+spikes.stim + spikes.rec,data=df.plt.pt)
      df.plt.pt$rho.spikes.stim.pr <- get.partial.resids(m.rho.spikes,'spikes.stim')$y
      df.plt.pt$rho.spikes.rec.pr <- get.partial.resids(m.rho.spikes,'spikes.rec')$y
      
      results.stim$p[[wave]]$rho.spikes[[pt]] <- get.coef.p.val(m.rho.spikes,'spikes.stim')
      results.stim$b[[wave]]$rho.spikes[[pt]] <- get.coef.beta(m.rho.spikes,'spikes.stim')
      results.rec$p[[wave]]$rho.spikes[[pt]] <- get.coef.p.val(m.rho.spikes,'spikes.rec')
      results.rec$b[[wave]]$rho.spikes[[pt]] <- get.coef.beta(m.rho.spikes,'spikes.rec')
...
plot.cfg <- list(list(ttl='HippocampalCLCorrVsSpikes',
                      p.ttl= 'Rho',
                      m.name='rho.spikes',
...
for(cfg in plot.cfg){
  for(wave in waveforms){
    ...
    df.p.rec.wave$values <- p.signif(df.p.rec.wave$values)
    df.p.stim.wave$values <- p.signif(df.p.stim.wave$values)
    ...
    p[[wave]][[cfg$ttl]] <- ggplot(df.b.wave,aes(x=pt,y=values,fill=stim.rec)) +  theme_classic() + 
      ...
      geom_text(data=df.p.rec.wave,aes(label=values,y=Inf,x=pt),color=wes_palettes$Darjeeling1[2],vjust=1,size=1.5) +
      geom_text(data=df.p.stim.wave,aes(label=values,y=-Inf,x=pt),color=wes_palettes$Darjeeling1[4],vjust=-1,size=1.5) +
    ...
    ggsave(plot = p[[wave]][[cfg$ttl]],filename = paste0(savedir,cfg$ttl,wave,'.pdf'),width = 4,height=4.5,units= 'cm',useDingbats=FALSE)
