...
assemble.reg.tab <- function(m,dep.var,pt,wave){
  ...
  var.names <- c('(Intercept)','Distance','Gray Matter (Stim)','Gray Matter (Record)','Spikes (Stim)','Spikes (Record)')
...
for(wave in waveforms){
  ...
  for(pt in pts){
    ...
    if(nrow(df.plt.pt)>0 & !all(is.na(df.plt.pt$spikes.rec))){
      ...
      m.ccep.spikes <- lm(log10(ccep)~log10(D)+gm.stim+gm.rec+spikes.stim + spikes.rec,data=df.plt.pt)
      m.std.spikes <- lm(std.trials~log10(D)+gm.stim+gm.rec+spikes.stim + spikes.rec,data=df.plt.pt)
      
      reg.tab.rho[[pt]] <- assemble.reg.tab(m.rho.spikes,'Rho',pt,wave)
      reg.tab.rho[[pt]] <- cbind(reg.tab.rho[[pt]],'') # add blank row to horizontally concatenate
      reg.tab.ccep[[pt]] <- assemble.reg.tab(m.ccep.spikes,'CCEP Amplitude',pt,wave)
      reg.tab.ccep[[pt]] <- cbind(reg.tab.ccep[[pt]],'') # add blank row to horizontally concatenate
      reg.tab.std[[pt]] <- assemble.reg.tab(m.std.spikes,'CCEP Amplitude Standard Deviation',pt,wave)
      reg.tab.std[[pt]] <- cbind(reg.tab.std[[pt]],'') # add blank row to horizontally concatenate
      
    }
  }
  reg.tab.rho <- do.call(cbind,reg.tab.rho)
  write.table(reg.tab.rho,file = paste0(savedir,'RegressionTable_',wave,'RhoSpikes_Fig5def.csv'),row.names = F,col.names = F,sep=',')
  reg.tab.ccep <- do.call(cbind,reg.tab.ccep)
  write.table(reg.tab.ccep,file = paste0(savedir,'RegressionTable_',wave,'CCEPAmplitudeSpikes_Fig5def.csv'),row.names = F,col.names = F,sep=',')
  reg.tab.std <- do.call(cbind,reg.tab.std)
  write.table(reg.tab.std,file = paste0(savedir,'RegressionTable_',wave,'CCEPStandardDeviationSpikes_Fig5def.csv'),row.names = F,col.names = F,sep=',')
}
