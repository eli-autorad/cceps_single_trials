function [all_traces,data] = get_stim_trials_sch(out,sch,art_reject)

% INPUTS:
% out: output struct
% sch: index of stim channel
% art_reject: logical indicating whether to perform artifact rejection. Set
% to false if you want to calculate pre-stim metrics only
%
% OUTPUTS:
% all_traces: time x num trials x electrode matrix of time series data
% from all trials for given stim electrode for all recording electrodes
%
% nans are added if max abs val is too high
if ~exist('art_reject','var')
    art_reject = true;
end
%% Parameters
do_bipolar = 0;
...
% Get alt avg
all_traces = nan(bits(1,end)-bits(1,1)+1,nt,nch);

for t = 1:nt % loop through trials
    if do_bipolar
        vals = bipolar_montage(values(bits(t,1):bits(t,end),:),[],chLabels);
    else
        vals = values(bits(t,1):bits(t,end),:);
    end
    ...
    % exclude trial for any channel with time points with absolute value > 1e3 (done to all electrodes)
    excl_channels = nanmax(abs(vals)) > 1e3;
    
    if art_reject
        vals(:,excl_channels) = nan;
    end
    %}
    all_traces(:,t,:) = vals; 
