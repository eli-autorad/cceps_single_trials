function [CCEPsParcellated,allElectrodesROIAssignments,electrodesInsideThreshold,allROIElectrodeAssignments,nElecsPerParcel] = ...
    CCEPS_TO_PARCELS_MNI(ElectrodeMNICoords,CCEPS,nifti,hemi_idx,dist_thresh,count)
...
% 
% count: if true, don't average data - sum it, so that you're counting binary values for each electrode
%
% OUTPUTS:
% 
...
% allROIElectrodeAssignments: cell with element for each parcel containing indices of electrodes in that parcel
% nElecsPerParcel: vector containing number of electrodes per parcel


if ~exist('dist_thresh','var')
    dist_thresh = 3; % distance threshold in mm for how close a probe must be to ROI to be assigned
elseif isempty(dist_thresh)
    dist_thresh = 3;
end
...
dist_thresh = dist_thresh / nifti.pixdim(2); % convert distance threshold to number of voxels
%dist_thresh = floor(dist_thresh);
...
for ROI = 1:nparc
    ...
    if ~isempty(allROIElectrodeAssignments{ROI}) % if there are electrodes in an ROI
        ...
        CCEPsParcellated_tmp(ROI,:) = ...
            fun(CCEPS(allROIElectrodeAssignments{ROI},:),1);
    end
end
...
for ROI = 1:nparc
    if ~isempty(allROIElectrodeAssignments{ROI}) % if there are electrodes in an ROI
        ...
        CCEPsParcellated(:,ROI) = ...
            fun(CCEPsParcellated_tmp(:,allROIElectrodeAssignments{ROI}),2);
    end
end

nElecsPerParcel = cell2mat(cellfun(@length,allROIElectrodeAssignments,'UniformOutput',false));
