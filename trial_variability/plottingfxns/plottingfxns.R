...
p.hist.div <- function(x,ttl,col){
  p <- ggplot() + geom_histogram(aes(x=x),fill = wes_palettes$Moonrise3[col],alpha=0.5) + theme_classic() +
    scale_y_continuous(expand = c(0,0),limits=c(0,1100)) + scale_x_continuous(limits=c(0,10)) +
    xlab(expression(rho)) + ylab('# of Cortical Regions') +
