...
for ich = 1:length(elecs)
    ...
    for jch = 1:size(elecs(ich).avg,2)
        ...
        [pks,locs] = findpeaks(n1_z_score,'MinPeakDistance',5e-3*fs); % look for local maxima                
        ...
        [pks,locs] = findpeaks(n2_z_score,'MinPeakDistance',5e-3*fs); % look for local maxima
